...
#define MCP2515_OPCODE_READ_RX_BUFFER_RXB1D0	0x06					// receive buffer 0, start at RXBB1D0, datasheet p. 67
#define MCP2515_OPCODE_WRITE			0x02							// write instruction, datasheet p. 65
#define MCP2515_OPCODE_LOAD_TX_BUFFER	0x40							// load TX buffer + bit fields, datasheet p. 65
#define MCP2515_OPCODE_LOAD_TX_BUFFER_TXB0SIDH	0x00					// TX buffer 0, start at TXB0SIDH, datasheet p. 68
#define MCP2515_OPCODE_LOAD_TX_BUFFER_TXB0D0	0x01					// TX buffer 0, start at TXB0D0, datasheet p. 68
#define MCP2515_OPCODE_LOAD_TX_BUFFER_TXB1SIDH	0x02					// TX buffer 1, start at TXB1SIDH, datasheet p. 68
#define MCP2515_OPCODE_LOAD_TX_BUFFER_TXB1D0	0x03					// TX buffer 1, start at TXB0D0, datasheet p. 68
#define MCP2515_OPCODE_LOAD_TX_BUFFER_TXB2SIDH	0x04					// TX buffer 2, start at TXB2SIDH, datasheet p. 68
#define MCP2515_OPCODE_LOAD_TX_BUFFER_TXB2D0	0x05					// TX buffer 2, start at TXB0D0, datasheet p. 68
#define MCP2515_OPCODE_RTS				0x80							// RTS command + bit fields; p. 65 chap 12.7 & figure 12.6 on p. 68
#define MCP2515_OPCODE_READ_STATUS		0xA0							// single instruction read of status bits, datasheet p. 65
#define MCP2515_OPCODE_RX_STATUS		0xB0							// determines received message filter and type, datasheet p. 66
#define MCP2515_OPCODE_BIT_MODIFY		0x05							// instruction for modification of single bits in selected registers: shaded fields in table 11.1, datasheet p. 66
...
#define CLKPRE0		0													//
#define REQOP_NORMAL		0x00										// normal operation mode, datasheet p. 60
#define REQOP_SLEEP			0x20										// sleep mode, datasheet p. 59/60
#define REQOP_LOOPBACK		0x40										// loopback mode, datasheet p. 60
#define REQOP_LISTENONLY	0x60										// listen-only mode, datasheet p. 59/60
#define REQOP_CONFIG		0x80										// configuration mode, datasheet p. 59/60
...
// public methods
	...
	void (*Sleep)(struct can_t *in_can, const uint8_t in_val);			// puts the MCP2515 to sleep (and wakes it up)
	void (*RequestToSend)(volatile can_msg_t * const msg);				// requests message to be sent to the CAN bus
