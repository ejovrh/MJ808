...
typedef struct															// message_handler_t actual
{
	...
	can_msg_t _msg;														// private - CAN message object
	can_t *_can;														// private - pointer to can_t struct
} __message_handler_t;
...
{
	__MsgHandler._msg.sidh = __MsgHandler._can->own_sidh;
	__MsgHandler._msg.sidl = __MsgHandler._can->own_sidl;

	if (in_command == CMND_ANNOUNCE)									// if we have the broadcast command
		__MsgHandler._msg.sidh |= BROADCAST;							//	then set the broadcast flag

	__MsgHandler._msg.COMMAND = in_command;								// set command into message
	__MsgHandler._msg.ARGUMENT = in_argument;							// set argument into message
	__MsgHandler._msg.dlc = in_len;										// set DLC

	// TODO - move sleep wakeup into _mcp2515_can_msg_send()
	__MsgHandler._can->Sleep(0);										// attempt to wake it up
	__MsgHandler._can->RequestToSend(&__MsgHandler._msg);				// load message into TX buffer and request to send
};
...
{
	__MsgHandler._can->FetchMessage(&__MsgHandler._msg);				// fetch the message from some RX buffer into RAM
	// FIXME - 1st LU doesn't always get listed in devices.All -- very likely the root cause in the quick'n'dirty arduino LU
	__MsgHandler.public.Devices |= ( 1 << ( (__MsgHandler._msg.sidh >> 2) & 0x0F ) );// populate devices in canbus_t struct so that we know who else is on the bus

	return &__MsgHandler._msg;											// return pointer to it to someone who will make use of it
};
...
{
	__MsgHandler._can = in_can;											// save address of can_t struct in private data member
