...
{
	__LED.flags = value;												// mark mj808 device as either on or off

	_fade(value, &OCR_FRONT_LIGHT);										// fade front LED to value
};
...
{
	__LED.flags = 1;													// mark mj808 device as on
	...
	MsgHandler->SendMessage(MSG_BUTTON_EVENT_BUTTON0_ON, 0x00, 1);					// convey button press via CAN and let other devices do their own thing
...
{
	__LED.flags = 0;													// mark mj808 device as off
...
static __composite_led_t __LED =
{
	...
	.public.Shine = &_component_led_mj808,								// component part ("interface") - accessed only via button event (not CAN direct messages)
	.flags = 0															// boolean flag which marks whole device mj808 as on or off
};

// implementation of virtual constructor for LEDs
static composite_led_t *_virtual_led_ctorMJ808()
{
	__LED.public.led[Utility].Shine = &_util_led_mj808;					// specific implementation for utility LED
	__LED.public.led[Front].Shine = &_wrapper_fade_mj808;				// specific implementation for front (high power) LED
