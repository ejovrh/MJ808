...
{
	  SpeedStopped,  // 0- stopped
	  SpeedbelowChargerThres,  // 1- below charger threshold
	  SpeedaboveChargerThres,  // 2- above charger threshold
	  Speed0,  // 3- standstill
	  Speed2,  // 4
	  Speed4,  // 5
	  Speed6,  // 6
...
static void _Do(void)  // this actually runs the AutoCharge application
{
	...
	uint32_t speed = Device->AutoDrive->mps.uint32;
	if(speed > 12000)
	...
	if(_CompareSpeedLevelsandFlag(__AutoCharge._SpeedLevel))	// if the speed flags have changed
		{
#if USE_TLC59208
			if(__AutoCharge._SpeedLevel != SpeedStopped)	// prevent premature power off
			...
			_SSR_SW2(__AutoCharge._SpeedLevel == Speed4 ? ON : OFF);  // NO to NC
			_SSR_SW1(__AutoCharge._SpeedLevel == Speed2 ? ON : OFF);  // NO to NC

#if USE_TLC59208
			__AutoCharge._LEDDriver->Write(0x0C, &_LEDword);  // write LED state to driver
			__AutoCharge._LEDDriver->Power(__AutoCharge._SpeedLevel ? 1 : 0);  // if movement, then turn on LED Driver
#endif
		}
}
