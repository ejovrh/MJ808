...
static inline void _CompareLightLevelsandFlag(const autodrive_lightlevels_t in_level)
{
	...
	_FlagSendLightlevelChangeEvent = 1;  // enable notification flag (see very first if-clause in Do())
}
...
static void _Do(void)  // this actually runs the AutoDrive application
{
	...
	_CompareLightLevelsandFlag(__AutoDrive._LightLevel);  // compares current and previous light levels and sets light level change flag
